...
const NAV = [
  ...
  {to:'/levels',  label:'LEVELS',    color:'#39FF14'},
  {to:'/characters',label:'RIDERS',  color:'#FF6EC7'},
  {to:'/packs',   label:'PACKS',     color:'#00EAFF'},
...
export default function Navbar() {
  ...
  return (
    <>
      ...
      `}</style>
      <nav style={{position:'sticky',top:0,zIndex:50,background:'rgba(5,10,14,0.97)',borderBottom:'1px solid rgba(184,134,11,0.2)',backdropFilter:'blur(12px)'}}>

        {/* Top bar */}
        <div style={{display:'flex',alignItems:'center',gap:10,padding:'5px 16px',borderBottom:'1px solid rgba(184,134,11,0.1)',flexWrap:'wrap'}}>
          ...
          {connected && address && <>
            ...
            <span style={{fontFamily:'Orbitron,monospace',fontSize:9,color:'#FFD700'}}>{
              parseFloat(balanceMOTO) > 9999
                ? (parseFloat(balanceMOTO)/1000).toFixed(1)+'K'
                : balanceMOTO
            } MOTO</span>
            ...
            <button onClick={addMotoToWallet} style={{
              ...
              color:'rgba(255,215,0,0.5)',background:'transparent',cursor:'pointer'
            }}>+MOTO</button>
          </>}
        </div>
          ...
          <div style={{flexShrink:0}}>
            {connected ? (
              <div style={{
                ...
                border:'1px solid #39FF14',background:'transparent',
                cursor:'pointer',
              }}
              onClick={disconnect}
